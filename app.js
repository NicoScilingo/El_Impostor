...
(function () {
    ...
    try {
      ...
      switch (action) {
        ...
        case 'nextRound':
          ...
          break;
        case 'resetVotes':
          // Cuando se reinician las votaciones se deben permitir los votos de nuevo
          voted = false;
          submitVoteBtn.disabled = false;
          // También deshacer cualquier confirmación de resultados
  ...
  async function pollRoom() {
    ...
    try {
      ...
      if (state === 'voting') {
        ...
        // Mostrar u ocultar botón de reiniciar votación sólo para el anfitrión
        resetVotesBtn.style.display = isHost ? 'inline-block' : 'none';
        resetVotesBtn.disabled = false;

        // Sincronizar la bandera 'voted' y el estado del botón con las votaciones actuales
        // Si ya no hay voto registrado en el servidor para este jugador, permitir votar.
        const hasVote = (votes || []).some(v => v.voterId === playerId);
        const currentPlayer = players.find(p => p.id === playerId);
        const alivePlayer = currentPlayer ? currentPlayer.alive : true;
        if (!hasVote) {
          voted = false;
          if (alivePlayer) {
            submitVoteBtn.disabled = false;
  ...
  function init() {
    ...
    // Reiniciar votación (sólo anfitrión): resetea los votos en el servidor y permite volver a votar
    resetVotesBtn.addEventListener('click', () => forceAction('resetVotes'));
